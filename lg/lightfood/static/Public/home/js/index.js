    ...
    var empile1 = new Empile($('.demo1>.container'), {
        ...
        on: {
            ...
            slideChange: function () {
                ...
                activeIndexEle.textContent = this.activeIndex + 2;
            }
        },
        container: {
            el: document.querySelector('.container'),
            clickable: true,
            bulletClass: 'cards-list',
        ...
        css: function (coord, absCoord) {
            var zIndex = 100+absCoord,
                opacity = Math.pow(1, absCoord).toFixed(3),
                scale = 'scale(' + Math.pow(1, absCoord).toFixed(2) + ')',
                translateX = 'translateX(' + 200 * coord + 'px)',
				borderRadius= 50+'%',
				marginLeft=390+'px',
				marginTop=600+'px',
				backgroundColor='#fff';
			...
			if(absCoord==0){
				borderRadius= 0+'%';
                translateX = 'translateX(' + 0 * coord + 'px)',
				marginLeft=0+'px',
				marginTop=0+'px',
				backgroundColor='';
				height=100+'%',
